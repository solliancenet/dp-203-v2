# Optimize a Data Warehouse Part 2

In this lab, we continue from the previous lab and delve into more advanced optimization topics. First, we cover how to understand table space usage and column store storage details. Next, we compare storage requirements between identical tables that use different data types. Finally, we observe the impact materialized views have when executed in place of complex queries, and learn how to avoid extensive logging by optimizing delete operations. The following table of contents describes and links to the elements of the lab:

- [Optimize a Data Warehouse Part 2](#optimize-a-data-warehouse-part-2)
